...
// coords define
#define CHECKPOINT_LAT 00.00000   // actual coords redacted !
#define CHECKPOINT_LNG 00.00000
#define FINAL_LAT 00.0000
#define FINAL_LNG 00.00000

#define COMBINATION XXX // combination to the three number lock box code. actual combination redacted !

#define UNIQUE_BUTTON_HOLD_TIME 7000  // this is the amount of time (ms) the button needs to be held to pass the 1st first checkpoint (when in proximity of the checkpoint location)

#define milesInKm 0.62137119223
#define feetInMile 5280.0

const double checkpointTarget[2] = {CHECKPOINT_LAT, CHECKPOINT_LNG};
const double finalTarget[2] = {FINAL_LAT, FINAL_LNG};
...
//  EEPROM.write(0,0);
  ...
  else { // game over from here on out forever
...
void loop() {
    ...
    if (journey_status == XMAS_MORN && digitalRead(buttonPin) == HIGH) { // xmas morning initial button press
    ...
    while (journey_status == CHECKPOINT_ARRIVED && digitalRead(buttonPin) == HIGH) {
      if (millis() - button_press_instant > UNIQUE_BUTTON_HOLD_TIME) { // checkpoint complete!!
        ...
        // update destination and journey status
        ...
        // update display with the limited amount of characters we have to work with on this tiny display
        u8x8.clearLine(4);
        u8x8.clearLine(5);
        u8x8.clearLine(6);
        u8x8.clearLine(7);
    ...
    if (gps.location.isUpdated()){
      digitalWrite(fixLed, HIGH); // GPS led indicator
      Serial.println();

      // get location and distance from gps data
      ...
      if (f < 300) {
        ...
        if (journey_status == NO_PROGRESS) { // arrived at checkpoint but haven't completed it yet
          ...
          u8x8.print("CONGRATS");
          u8x8.setCursor(0,2);
          u8x8.print("NICKY & LIV!!!"); // this should be dynamically populated
          u8x8.refreshDisplay();
          ss.end();
          arrivalRoutine();
          cur_status = ARRIVED;
